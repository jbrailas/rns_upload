...
* @license		GNU/GPLv2
* @modified		Giannis Brailas - PPCR IT Department
*/
...
class rnsuploadFileupload 
{
		...
		if (isset($fparams[2]["name"]))
		{
			...
			//$result['show'] = true;
				...
				if ($fparams[2]["error"] > 0)
				{
					...
					$error_text = JText::_('MOD_EFU_RETURN_CODE').": ".$fparams[2]["error"]."<br />";
				...
				else
				{
					...
					if (rnsuploadFileupload::isValidFileType($fparams, $i))
					{
						...
						if ($fparams[2]["size"] < $fparams[3])
						{
							...
							/*	if ($fparams[4] == true && $_POST["answer"] == true) */
								if ($fparams[4] == true)
								{
									...
									JFile::delete($path.DIRECTORY_SEPARATOR  . (trim(preg_replace('/[&\/\\#,+()$~%:*?<>{}]/', '', $fparams[2]["name"]), '()')));
								...
								{
									$result['type'] = 'warning';
									$result['text'] = (trim(preg_replace('/[&\/\\#,+()$~%:*?<>{}]/', '', $fparams[2]["name"]), '()'))." ".JText::_('MOD_EFU_ALREADY_EXISTS');
						...
						else
						{
							//file is too large
							$result['type'] = 'warning';
							$result['text'] = JText::_('MOD_EFU_TOO_LARGE_ERROR'). " MAX SIZE: " . rnsuploadFileupload::sizeToText($fparams[3]).".";
					...
					else
					{
						...
						$result['type'] = 'error';
						$result['text'] = JText::_('MOD_EFU_INVALID_ERROR')."<br />".JText::_('MOD_EFU_PHP_MIME_ERROR').($trueMIME!==false?"(".$trueMIME.")":"")."<br />".JText::_('MOD_EFU_BROWSER_MIME_ERROR').$fakeMIME;
					}
				}
			//} // end for
		}
	...
	private static function storeUploadedFile($filepath, &$fparams, &$result, &$i, $replaced = false)
	{
		...
		if ($success)
		{
			...
			$result_text.= JText::_('MOD_EFU_NAME').": ".(trim(preg_replace('/[&\/\\#,+()$~%:*?<>{}]/', '', $fparams[2]["name"]), '()'))."<br />";
			$result_text.= JText::_('MOD_EFU_TYPE').": ".$fparams[2]["type"]."<br />";
			$result_text.= JText::_('MOD_EFU_SIZE').": ".rnsuploadFileupload::sizeToText($fparams[2]["size"])."<br />";
	...
	public static function checkFileSafety(&$fparams, &$result, &$i, $forbidden = array('php', 'phps', 'php5', 'php3', 'php4', 'inc', 'pl', 'cgi', 'fcgi', 'java', 'jar', 'py'))
	{
		...
		{
			$result['type'] = 'error';
			$result['text'] = JText::_('MOD_EFU_NULL_BYTE_FOUND');
		...
		foreach ($forbidden as $script)
		{
			...
			{
				$result['type'] = 'error';
				$result['text'] = JText::_('MOD_EFU_FORBIDDEN_SCRIPT_FOUND');
		...
		if ($fp !== false)
		{
			...
			while (!feof($fp) && $safe === true)
			{
				...
				{
					$result['type'] = 'error';
					$result['text'] = JText::_('MOD_EFU_PHP_TAG_FOUND');
				...
				if ($is_script)
				{
					...
					{
						$result['type'] = 'error';
						$result['text'] = JText::_('MOD_EFU_SHORT_TAG_FOUND');
				...
				if (!$allow_scripts_in_archive)
				{
					...
					if ($is_archive)
					{
						foreach ($forbidden as $ext)
						{
							...
							{
								$result['type'] = 'error';
								$result['text'] = JText::_('MOD_EFU_FORBIDDEN_IN_ARCHIVE_FOUND');
